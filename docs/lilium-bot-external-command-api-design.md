...
无状态 HTTP 命令中，`reply` 回复本次 slash command 消息。Stateful WebSocket
命令中，`reply` 回复启动该 session 的命令消息。v1 如果需要响应某条
`session.input` 消息，应使用 `send_text`。
...
v1 使用保守策略：某个 effect 执行失败后，Lilium 会停止执行后续 effects，并将本次调用标记为失败或部分失败。
...
- 同一次 invocation 的 Bot 侧重试会复用同一个 `Idempotency-Key`。
- 用户重新发送同一个命令会产生新的 `invocation_id` 和新的 `Idempotency-Key`。
...
`(local_session_id, timer_id, fire_at)` 做幂等处理。
...
- 如果一个 effect 执行失败，Lilium 不会继续执行后续排队 effects，直到第三方发送修复性的更新或关闭 session。
- Stateful 模式下 `reply` 回复启动 session 的命令消息；响应后续 `session.input` 时，v1 应使用 `send_text`。
...
`/stop` 由 Lilium 本地处理。Lilium 会尽力通知第三方，并在 v1 中等待最多
5 秒接收 final effects 或 `session.closed`。无论第三方是否可达，Lilium 都必须释放本地
listener、timer 和房间 mutex。
...
第三方随后返回 `session.started` 或 `session.closed`。`session.started` 中的
`listen_rules` 和 `timers` 是恢复后的权威监听状态，会替换 Lilium 本地持久化的旧值；
空数组表示没有活跃 listener 或 timer。
